...
ScopeDefinition.prototype.related = function(receiver, scopeParams, condOrRefresh, options, cb) {
  ...
  if (!self.__cachedRelations || self.__cachedRelations[name] === undefined ||
    ...
    if (scopeOnRelatedModel) {
      ...
      var filterScope = filter.scope;
      var queryRelated = {where: {}};
      queryRelated = Object.assign(queryRelated, filterScope);
      delete params.include.scope;
      // ['limit', 'offset', 'skip', 'order'].forEach(function(pagerFilter) {
      //   if (typeof(filterScope[pagerFilter]) !== 'undefined') {
      //     queryRelated[pagerFilter] = filterScope[pagerFilter];
      //     delete params.include.scope[pagerFilter];
      //   };
      // });
    };
    ...
    targetModel.find(params, options, function(err, data) {
      ...
      if (scopeOnRelatedModel === true) {
        ...
        // hardcode as id, need to find the function
        var IdName = 'id';
        queryRelated.where[IdName] = collectTargetIds(data, IdName);
        relatedModel.find(queryRelated, function(err, filteredData) {
          cb(err, filteredData);
        });
      } else {
        cb(err, data);

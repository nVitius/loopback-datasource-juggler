...
Memory.prototype.fromDb = function(model, data) {
  ...
  var props = this._models[model].properties;
  function cast(prop, val, props) {
    if (Array.isArray(val)) {
      return val.map(function(val) {
        return cast(prop, val, props);
      });
    }

    var isArray = Array.isArray(props[prop].type);
    var name = isArray ? props[prop].type[0].name : props[prop].type.name;
        ...
        for (var subProp in val) {
          val[subProp] = isArray ?
            cast(
              subProp,
              val[subProp],
              props[prop].type[0].definition.properties
            ) :
            cast(subProp, val[subProp], props[prop].type.definition.properties);
        }
        break;
    }
